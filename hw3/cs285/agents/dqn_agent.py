...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        perform_random_action = np.random.choice([True,False], p=np.array([eps, 1.-eps]))
        if self.learning_starts > self.t:
        ...
        if perform_random_action:
            ...
            action = np.random.choice(np.arange(self.critic.ac_dim))  # how does the action space look like?!
        else:
